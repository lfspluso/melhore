...
| Settings UI | `feature/settings/` – `ui/` (includes delete after completion setting - Sprint 13, snooze options settings - Sprint 15, Sign out - Sprint 17) |
| Database (entities, DAOs, DB) | `core/database/` – `entity/` (Reminder with ReminderStatus enum - Sprint 13, Category, List, ChecklistItem - all have userId - Sprint 15), `dao/` (all filter by userId), `MelhoreDatabase.kt` (version 6 with migrations 1→6; migration 5→6 adds indexes for query optimization - Sprint 12.1), `DatabaseModule.kt` |
...
| App preferences (default snooze, filter/sort, etc.) | `core/common/` – `preferences/AppPreferences.kt` (includes showAdvancedFilters - Sprint 10, showCompletedReminders - Sprint 13, deleteAfterCompletion - Sprint 13, enabledSnoozeOptions - Sprint 15) |
